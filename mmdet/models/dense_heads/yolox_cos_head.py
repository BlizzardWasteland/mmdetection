...
class NormConv2d(nn.Conv2d):
    def __init__(self, **kwargs):
        ...
        self.reset_parameters_sigma()

    def reset_parameters_sigma(self):
        if self.sigma is not None:
            self.sigma.data.fill_(1) #for initializaiton of sigma

    def forward(self, input):
        out = self._conv_forward(F.normalize(input, p=2,dim=1), F.normalize(self.weight, p=2, dim=1), self.bias)
        if self.sigma is not None:
            out = self.sigma * out
        return out
...
class YOLOXCosHead(BaseDenseHead, BBoxTestMixin):
    ...
    def _build_predictor(self):
        ...
        conv_cls = NormConv2d(in_channels=self.feat_channels, out_channels=self.cls_out_channels, kernel_size=1, bias=False)
        conv_reg = nn.Conv2d(self.feat_channels, 4, 1)
        conv_obj = nn.Conv2d(self.feat_channels, 1, 1)
        return conv_cls, conv_reg, conv_obj

    def init_weights(self):
        super(YOLOXCosHead, self).init_weights()
