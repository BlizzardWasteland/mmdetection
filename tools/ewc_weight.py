...
def main():
    ...
    # build_dataloader(ds, **train_loader_cfg)
    ...
    path = osp.join(cfg.work_dir, 'ewc_weight.pth')
    # path = args.work_dir + '/ewc_weight.pth'
    ...
    for i, data in enumerate(data_loader):
        ...
        # loss = model(return_loss=False, rescale=True, **data)
        optimizer.zero_grad()
        loss = model.val_step(data)['loss']
        # loss = model(return_loss=True, **data)
        loss.backward()
        
        for name, p in model.named_parameters():
            g = p.grad.clone().detach().cpu()
            if 'module' in name:
                weight[name[7:]] += g**2
            else:
                weight[name] += g**2
    ...
    # osp.join(cfg.work_dir, f'{timestamp}.log')
