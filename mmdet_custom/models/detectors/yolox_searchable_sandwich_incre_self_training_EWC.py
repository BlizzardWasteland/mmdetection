...
class SearchableYOLOX_KD_Incre_ST_EWC(SearchableYOLOX_KD):
    def __init__(self,
                 ori_num_classes,
                 ori_checkpoint_file,
                 ori_config_file,
        ...
        self.lamda = lamda
        # self.ori_weighted_file = ori_weighted_file
        self.weight_ewc = torch.load(ori_weighted_file, map_location='cpu')
        keys = list(self.weight_ewc.keys())
        for k in keys:
            if 'module.' == k[:7]:
                self.weight_ewc[k[7:]] = self.weight_ewc[k]
                del self.weight_ewc[k]
        self.smooth = smooth
        if smooth:
            self.huber_loss = nn.HuberLoss(reduction='sum', delta=beta)
        # self.beta = beta
    ...
    def load_checkpoint_for_new_model(self, checkpoint_file, map_location='cpu', strict=False, logger=None):
        ...
        for i in range(len(self.bbox_head.multi_level_conv_cls)):
            ...
            if self.bbox_head.multi_level_conv_cls[i].bias is not None:
    ...
    def init_detector(self, config, checkpoint_file):
        ...
        self.ori_model_p = ori_model.state_dict()
    ...
    def get_ewc_loss(self):
        ...
        for name, p in self.named_parameters():
            if 'ori_model' in name:
                continue 
            if self.encoder_only and 'bbox_head' in name:
                continue
                
            p0 = self.ori_model_p[name].cuda(p.device).detach()
            w = self.weight_ewc[name].cuda(p.device).detach()
            # print(p0, w, p)
            if 'multi_level_conv_cls' in name:
                p1 = p[:self.ori_num_classes, ...]
            else:
                p1 = p

            if not self.smooth:
                loss += (w * torch.pow(p1 - p0, 2)).sum()
            else:
                w0 = torch.sqrt(w)
                loss += self.huber_loss(p1*w0, p0*w0)
